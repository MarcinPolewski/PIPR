...
def write_to_csv(file_handle, file_info_classes):
    ...
    writer.writeheader()
    for fi_class in file_info_classes:
        writer.writerow(
            {
                "path": fi_class.path,
                "parent_folder_path": fi_class.parent_folder_path,
                "parent_folder_name": fi_class.parent_folder_name,
                "file_name": fi_class.file_name,
                "file_ext": fi_class.file_ext,
                "number_of_lines": fi_class.count_lines(),
            }
        )
...
def read_from_csv(file_handle):
    ...
    for row in reader:
        read_classes.append(FileInfo(row["path"]))
...
def main():
    ...
    try:
        with open("classes.csv", "w") as file_handle:
            write_to_csv(file_handle, file_info_classes)
    except Exception:
        print("dupa1")

    read_classes = []
    try:
        with open("classes.csv", "r") as file_handle:
            pass
            read_classes = read_from_csv(file_handle)
