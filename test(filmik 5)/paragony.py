def split_price(price):
    try:
        ...
        raise ValueError("price could not be converted to int")
    is_negative = price < 0
    if is_negative:
        price = -price
...
def get_description(name, price):
    if not name:  # jeśli name jest puste
        raise ValueError("komunikat, ktory idzie razem z wyjatkiem")
...
def print_receipt(receipt):
    ...
    for i in range(len(receipt)):
        print(
            f"{i:3}. {receipt[i][0]:15}\t{format_price(receipt[i][1]):>6} {get_tax_group(receipt[i][0])}"
        )
    print("-" * 30)
...
my_receipt = [("Bananas", 499), ("Oranges", 1803), ("Milk", 315)]
